...
export default function HomePage() {
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const req: GenerateRequest = { subjectId, grade, knowledgePointIds: selectedKPs, questionTypes: selectedTypes, counts };
      const result = await api.generate(req);
      nav(`/preview/${result.id}`);
    } catch (e) {
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="flex flex-wrap gap-2">
            {subjects.map((s) => (
              <button
                key={s.id}
                onClick={() => { setSubjectId(s.id); setGrade(''); setSelectedKPs([]); }}
          ...
          <section className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {currentSubject.grades.map((g) => (
                <button
                  key={g.grade}
                  onClick={() => { setGrade(g.grade); setSelectedKPs([]); }}
          ...
          <section className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {kpoints.map((kp) => (
                ...
                >
                  {kp.name}
                  {kp.hasChart && <span className="ml-1 text-xs opacity-70">📊</span>}
                </button>
              ))}
            </div>
          </section>
